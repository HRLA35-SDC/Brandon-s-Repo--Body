...
// const writeData = () => {
//   let data = generateRandomDataSet(500)
//   let counter = 1
//   data.forEach(shoe => {
//     shoe.nikeID = counter
//     counter++
...
const writeData = async () => {
  await fs.writeFile(`${__dirname}/data.json`, `[`, (err) => {
    if (err) console.error('FUCKED UP', err)
  })
  for (var i = 0; i < 10000000; i++) {
    let data = generateRandomDataSet(1)
    data[0].nikeID = i;
    await fs.appendFileSync(`${__dirname}/data.json`, `, ${JSON.stringify(data[0])}`, (err) => {
      if (err) console.error('FUCKED UP', err)
    })
  }
  await fs.appendFileSync(`${__dirname}/data.json`, `]`, (err) => {
    if (err) console.error('FUCKED UP', err)
  })
}
